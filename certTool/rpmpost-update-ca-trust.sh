...
#
# Scripts that adds/removes CA-TRUSTED-SSL-CERT into/from system-wide
# trusted certificates.
# The script checks if CA-TRUSTED-SSL-CERT is present
# in /etc/pki/certTool. If it's found then it's symlinked. If it's not found
# then it's ensured the symlink does not exists. Finally the trust update
# is run.
# Intended to run as post script in ca-trusted-ssl-cert-*.rpm
#

CERT_DIR=/etc/pki/certTool
CERT_FILE=CA-TRUSTED-SSL-CERT
...
if [ -d /etc/pki/ca-trust/source/anchors ]; then
    ...
    if [ -f $CERT_DIR/$CERT_FILE ]; then
        ln -sf $CERT_DIR/$CERT_FILE $TRUST_DIR/${CERT_FILE}.pem
        if [ $(grep -- "-----BEGIN CERTIFICATE-----" $CERT_DIR/$CERT_FILE | wc -l) -gt 1 ]; then
            csplit -b "%02d.pem" -f $TRUST_DIR/${CERT_FILE}- $CERT_DIR/$CERT_FILE '/-----BEGIN CERTIFICATE-----/' '{*}'
        fi
    fi
